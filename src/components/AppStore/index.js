...
class AppStore extends Component {
  ...
  filteredApps = activeTabId =>
    appsList.filter(eachApp => eachApp.category === activeTabId)

  onChangeSearch = event => {
    const sequence = event.target.value
    this.setState({searchSequence: sequence})
  }

  filterAppsOnSequence = (filteredApps, searchSequence) =>
    filteredApps.filter(each =>
      each.appName.toLowerCase().includes(searchSequence.toLowerCase()),
    )

  render() {
    const {activeTabId, searchSequence} = this.state
    let filteredApps = this.filteredApps(activeTabId)
    filteredApps = this.filterAppsOnSequence(filteredApps, searchSequence)
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              handleOnTabClick={this.handleOnTabClick}
